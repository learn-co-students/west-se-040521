...
- Rake
  ...
  ***
- File structure
  - `Gemfile` a list of gems used by the app; `bundler` uses this file to install dependencies; similar to `package.json` in Javascript
...
- Model.last
  - instance with the highest ID in the db
- Model.find
...
- Model.find_by\({ attribute: value }\)
  ...
  ## CFU: CR with AR (Creating and Reading db records with Active Record)
  * Add a new column/attribute of a `club` (string) for Trainers. (Check the section on Migrations in the guides linked below.)
    * The above is a 2-step process
  * using your `rake console` task, do the following
  * `create` a new Trainer in the database
  * `create` 2 new Boxers which belong to the new trainer
  * use AR to get an array of all the new trainer's boxers
  * use AR to get the instance of the last boxer's trainer
